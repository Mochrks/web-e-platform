...
export function evaluateAnswer(
  ...
  const keywordMatch =
    keywords.length > 0 ? (matchedKeywords.length / keywords.length) * 100 : 50;

  // Depth analysis (based on length and structure)
  const depthScore = Math.min(
    100,
    (wordCount / minWordCount) * 50 + (answer.split('.').length - 1) * 10
  );
...
export function evaluateCode(
  ...
  const efficiency = Math.min(
    100,
    (usesBuiltInMethods ? 50 : 30) + (hasNestedLoops ? 20 : 50)
  );
